use crate::{
    api::{
        ...
        tmdb::{movies::models::MovieVideosResponse, utils},
    },
    error,
...
pub async fn get_movie_videos(
    ...
    // Make request
    let endpoint_url = match utils::parse_url(format!("/movie/{}/videos", movie_id).as_str()) {
        Ok(url) => url,
    ...
    match response.json().await {
        ...
        Err(err) => {
            error!(
                Category::Tmdb,
                "Parsing MovieVideosResponse failed with error: {:#?}", err
            );
